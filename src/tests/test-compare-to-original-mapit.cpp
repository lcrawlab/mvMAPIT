...
context("MAPIT1_Normal") {
    ...
    test_that("projection is equal to original") {
        ...
        arma::mat correct_answer = K - b * btb_inv * (b.t() * K) - (K * b) * btb_inv * b.t() + b * btb_inv * (b.t() * (K * b)) * btb_inv * b.t();
...
//         expect_true(result(0, 2) - correct_answer(0, 2) == 0.0);
    ...
    test_that("compute_q_vector is equal to original") {
        ...
        G.each_row() %= x_k;
        G.each_col() %= x_k.t();
        arma::mat Gc_original = G - b * btb_inv * (b.t() * G) - (G * b) * btb_inv * b.t() + b * btb_inv * (b.t() * (G * b)) * btb_inv * b.t();
        arma::mat M = compute_projection_matrix(n, b);
        arma::mat Kc = M * K * M;
        arma::mat Gc = M * G * M;
        arma::mat Yc = Y * M;
        ...
        correct_answer(2) = as_scalar(yc_original.t() * (arma::eye<arma::mat>(n, n)-(b * btb_inv) * b.t()) * yc_original);
