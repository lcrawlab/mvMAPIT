...
context("davies_routine") {
    test_that("davies_routine for q matrix") {
        ...
        arma::mat q(num_variance_components, num_combinations); q.ones();
        std::vector<arma::mat> matrices(num_variance_components);
        arma::mat M(num_samples, num_samples); M.eye();
        arma::mat S(num_variance_components, num_variance_components); S.eye();
        matrices = { M, M, M };
        arma::mat correct_answer = arma::eye(num_combinations, num_variance_components);
        // when
