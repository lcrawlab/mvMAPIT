...
#ifdef WITH_LOGGER
  std::string logname = "mqs.product_trace";
  auto logger = spdlog::get(logname);
  if (logger == nullptr)
    logger = spdlog::r_sink_mt(logname);
#endif
#ifdef WITH_LOGGER
  logger->info("Start and return.");
#endif
...
#ifdef WITH_LOGGER
  logger->info("Start.");
  logger->info("&t = {}", (void*) &t);
  logger->info("&t2 = {}", (void*) t2);
#endif
  int num_variance_components = matrices.size();
...
double compute_var_bilinear_approx(const arma::vec &y1, const arma::vec &y2,
                                   ...
                                   const arma::mat &V12) {
#ifdef WITH_LOGGER
  std::string logname = "mqs.mqs.compute_var_bilinear_approx";
  auto logger = spdlog::get(logname);
  if (logger == nullptr)
    logger = spdlog::r_sink_mt(logname);
  logger->info("Computing variance of covariance component.");
#endif
#ifdef WITH_LOGGER
  logger->info("Start and return.");
#endif
