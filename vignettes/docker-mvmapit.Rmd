...
vignette: >
  %\VignetteIndexEntry{Using the mvMAPIT Docker Image}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

In order to enable users to work with mvMAPIT without having to install all dependencies, we provide a docker image with
all dependencies and an installed R package `mvMAPIT`. Learn how to use it or build your own version of the docker image.
...
This will start the docker container that serves an RStudio application at [localhost:8787](localhost:8787). In this
container, `mvMAPIT` is already installed and can be imported in the R console via `library(mvMAPIT)`.

![RStudio with mvMAPIT.](docker-mvmapit.png){width=80%}
