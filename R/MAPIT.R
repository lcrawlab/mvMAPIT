...
#' @param X is the pxn genotype matrix where p is the number of variants and n is the number of samples. Must be a matrix and not a data.frame.
#' @param y1 is the nx1 vector of quantitative or continuous traits.
#' @param y2 is the nx1 vector of quantitative or continuous traits.
...
#'
#' @param vc.mod The return object of the MAPIT function.
#' @param X A vector containing names.
#' @return The p-values for association.
...
#' @param X is the pxn genotype matrix where p is the number of variants and n is the number of samples. Must be a matrix and not a data.frame.
#' @param y is the nx1 vector of quantitative or continuous traits.
#' @param W is the matrix qxn matrix of covariates. Must be a matrix and not a data.frame.
#' @param C is an nxn covariance matrix detailing environmental effects and population structure effects.
#' @param hybrid is a parameter detailing if the function should run the hybrid hypothesis testing procedure between the normal Z test and the Davies method. Default is TRUE.
#' @param threshold is a parameter detailing the value at which to recalibrate the Z test p values. If nothing is defined by the user, the default value will be 0.05 as recommended by the Crawford et al. (2017).
...
MAPIT <- function(X, y, W = NULL, C = NULL, hybrid = TRUE, threshold = 0.05, test = "normal", cores = 1) {
  return(MvMAPIT(X, y, y, W, C, hybrid, threshold, test, cores))
}
