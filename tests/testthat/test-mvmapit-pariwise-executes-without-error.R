...
0.5015375, 0.4619467, 0.1347061, 0.4507170, 0.6405290, 0.2410251),
...
test_that("test = davies. phenotypeCovariance = 'combinatorial'", {
  ...
  d <- 3
  pvalues <- matrix(c(
0.01624319,     0, 0.6531582, 1.380850e-04, 0.000000e+00, 0.419213,
0.02694842,     0, 0.3977345, 5.755901e-05, 1.216344e-08, 0.490887),
                    nrow = p, ncol = 6, byrow = TRUE)
  colnames(pvalues) <- c("P1*P1", "P2*P1", "P2*P2", "P3*P1", "P3*P2", "P3*P3")
  set.seed(853)
  X <- matrix(runif(p * n), ncol = p)
...
0.8920097, 0.9107812, 0.9787608, 0.6248188, 0.2751130, 0.4994958,
0.5868067, 0.5128342, 0.3823134, 0.8747280, 0.2352273, 0.6889640,
0.3184337, 0.5047131, 0.6774045, 0.3071930, 0.8257162, 0.5527816
                      ),
                    nrow = p, ncol = 6, byrow = TRUE)
  colnames(pvalues) <- c("P1*P1", "P2*P1", "P2*P2", "P3*P1", "P3*P2", "P3*P3")
  set.seed(29)
...
test_that("C is not NULL, test = 'davies'. phenotypeCovariance = 'combinatorial'", {
  ...
  d <- 3
  pvalues <- matrix(c(
0.13395441,     0,     0, 0.8361945, 0.001629745, 0.009014612,
0.00000000,     0,     0, 0.5526955, 0.000000000, 0.000000000,
0.01039137,     0,     0, 0.7475025, 0.000000000, 0.000000000,
0.00000000,     0,     0, 0.7418526, 0.000000000, 0.000000000
),
                    nrow = p, ncol = 6, byrow = TRUE)
  colnames(pvalues) <- c("P1*P1", "P2*P1", "P2*P2", "P3*P1", "P3*P2", "P3*P3")
  set.seed(853)
